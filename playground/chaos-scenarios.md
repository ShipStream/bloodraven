...
- The fence costs the reader nothing but its writability: both replication threads are still running 5s later. The second sample is past the sidecar's eviction deadline (15s), so whichever actor fenced, every `KILL` it was going to issue has happened or been abandoned before the last look. A fence that kills the site's own `system user` threads strands it — permanently, once it is diverged enough for source convergence to be `Blocked`.
