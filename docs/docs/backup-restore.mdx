...
- The sidecar's `PITRStatus` API field exists but is not yet
  populated by a controller-side manifest scan; live archiver health
  comes from the sidecar HTTP endpoint in the meantime.
