...
`Pending` with reason `NoHealthySource` and retries; a normal failover window
does not consume the one-shot backup as a terminal failure. Once the Job is
created, its selected source is pinned. Later reconciles observe that Job
without re-running source selection against a topology that may be changing.
