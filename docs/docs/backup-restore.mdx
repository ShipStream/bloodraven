...
`MYSQL_USER` / `MYSQL_PASSWORD` keys parsed from the failover group's
DSN secret) and mounts it **as files** at
...
`spec.backup.pitr` is reserved for a future release. When set to
`true` the reconciler emits a `BackupPITRNotImplemented` warning
event on every sync; no PITR logic is wired up today. The field is
in place so the API shape is stable for when PITR lands.
