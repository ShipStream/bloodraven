...
<img src={require('/static/img/infographics/operator-availability.webp').default} alt="operator availability infographic" />
...
| t<sub>r</sub> + ≈ 6 s + [relay drain](./failover.mdx#failover-sequence) (up to 30 s) | Operator promotes the replica, records the promotion, and best-effort flips the DNSEndpoint. | `FailoverExecuted` Event. MySQL is writable on the target; Service-label and taint convergence is still poll-driven. |
