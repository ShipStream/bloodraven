...
7. **Flip Cloudflare DNS**
   - Updates the DNS A-record to point to the candidate site's load balancer IP
...
To prevent rapid failover oscillation (e.g., a flapping network link), the operator enforces a cooldown period between automatic failovers. The default is 60 minutes, configurable via `spec.failoverCooldown`.
