...
For MySQL image changes specifically, including the 9.6 → 9.7 LTS transition, see the [Upgrade and version-skew policy](./upgrade-policy.mdx). The replica-first ordering above is the MySQL-required direction for a rolling version upgrade.
