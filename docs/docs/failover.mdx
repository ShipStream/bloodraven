...
:::danger
Divergent transactions mean the old primary accepted writes that the new primary never received. These transactions are effectively lost from the replication stream. The site must be **wiped and re-cloned** from the current primary to recover. Do not attempt to manually reconfigure replication — conflicting GTID sets will cause replication errors.
:::
...
2. Delete the PVC for the affected site to trigger a fresh data clone
3. The operator will detect the fresh instance and bootstrap it as a replica via `CLONE INSTANCE`
