...
Backup, restore, and verification Jobs get the same Secret mounted at `/etc/mysql/tls` and connect with `ssl-mode=VERIFY_CA` against its `ca.crt` — both the `mysqlsh` session and the `mysqlbinlog | mysql` PITR replay. If `ca.crt` is somehow absent from the pod they fall back to `REQUIRED` (encrypted but unverified) rather than failing the job.
