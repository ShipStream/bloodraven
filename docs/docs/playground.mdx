...
The playground also deploys RustFS (`rustfs/rustfs:latest`) as an in-cluster S3-compatible target and creates the `dragonfly` bucket. Dragonfly snapshots are configured to `s3://dragonfly/playground` with credentials from `dragonfly-s3-credentials` and endpoint `rustfs.bloodraven-playground.svc.cluster.local:9000`, so the D6a snapshot-restore upgrade path can run without external AWS resources.
...
Exercise the D6a snapshot-restore upgrade path against RustFS:

```bash
make chaos-run SCENARIO=29-dragonfly-snapshot-upgrade

# Or trigger it manually with the current image to force a snapshot/restart/restore cycle.
IMAGE=$(kubectl -n bloodraven-playground get mfg playground -o jsonpath='{.spec.dragonfly.image}')
kubectl -n bloodraven-playground annotate --overwrite mfg playground \
  bloodraven.shipstream.io/dragonfly-snapshot-upgrade="$IMAGE"

kubectl -n bloodraven-playground get mfg playground \
  -o jsonpath='{.status.dragonfly.upgrade.phase}{"\n"}'
```
