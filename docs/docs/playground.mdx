...
The playground `MysqlFailoverGroup` enables `spec.dragonfly`, so Bloodraven also creates one Dragonfly StatefulSet per site (`playground-dragonfly-iad`, `playground-dragonfly-pdx`) plus a single app-facing `playground-dragonfly` Service whose endpoints follow the active site via the `shipstream.io/dragonfly-role=master` label. Disable Dragonfly by removing the `spec.dragonfly` block.
