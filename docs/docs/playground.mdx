...
| `./playground/enable-encryption.sh` | Turning on [data-at-rest encryption](./encryption-at-rest.mdx) — generates playground TLS material and converts the group |

## Data-at-rest encryption in the playground

The playground comes up unencrypted. To exercise the keyring lifecycle:
...
The script generates a self-signed CA and server certificate (encryption
requires `spec.tls`, because MySQL mandates a secure connection to clone
encrypted data), patches the failover group, and waits for every site to
reach `phase: Sealed`.
...
non-default baseline. CI runs it explicitly in a dedicated encryption job
after executing `enable-encryption.sh`. See
