...
| `splitBrainPolicy` | SplitBrainPolicySpec | No | -- | Opt-in automated resolution when both sites are writable and the operator has no prior failover history. Omit or leave `preferSite` empty to require manual resolution (alert only). |
...
Opt-in configuration for how the operator should behave when both sites are simultaneously writable. See [Split-brain resolution](./failover.mdx#split-brain-resolution) for a full description of semantics and tradeoffs.

| Field | Type | Required | Description |
|---|---|---|---|
| `preferSite` | string | No | Site name that wins unresolvable split-brain ties. Must match one of `spec.sites[].name`. If empty or unset, the operator falls back to manual resolution (alert only). |
...
|---|---|
| `bloodraven.shipstream.io/reclone-site` | Triggers a `CLONE INSTANCE` from the current primary to the named site. Value format is `<site>` (cold reclone, when `status.sites[].divergentGtid` is empty) or `<site>:<gtid-prefix>` (when divergence is recorded — the prefix must be 8+ characters and match `status.sites[].divergentGtid`, as a confirmation interlock against fat-fingered site names). Invalid annotations are rejected with a `RecloneRejected` Warning Event and cleared. Valid ones emit `RecloneRequested` and report progress via the `Bootstrapping` condition. |
