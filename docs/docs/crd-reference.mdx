...
| `initFromBackup` | InitFromBackupSpec | No | -- | One-shot restore-on-first-boot before normal bootstrap completes. See [InitFromBackupSpec](#initfrombackupspec). |
| `restoreInPlace` | RestoreInPlaceSpec | No | -- | Re-triggerable destructive restore against the active primary. Bumping `confirm` to a newer RFC 3339 timestamp re-arms. See [RestoreInPlaceSpec](#restoreinplacespec). |
...
| `restoreInPlace` | RestoreInPlaceStatus | In-flight or completed in-place restore (phase, JobName, targetSite, scope, confirmTokenUsed, startTime, completionTime, message). Mirrors the state machine described on [RestoreInPlaceSpec](#restoreinplacespec). |
