...
| `template` | StandbyFailoverGroupTemplate | Yes | -- | Embedded `MysqlFailoverGroupSpec` the controller will materialize on activation (Phase 3). Declare this at standby-CR-create-time, not during an incident. |
| `freshness` | StandbyFreshnessSpec | No | -- | Discovery cadence, verification schedule, staleness threshold, and cursor-refresh interval. |
| `activate` | StandbyActivateSpec | No | -- | One-shot promote gate (Phase 3). Ignored until `spec.activate.confirm` is set to a fresh RFC 3339 timestamp. |
...
|---|---|---|---|
| `discoveryInterval` | duration | `5m` | How often the controller re-scans the bucket to refresh `status.discovered`. |
| `verifySchedule` | string | `"0 4 * * *"` | Cron expression for scheduled restore-verification runs (Phase 2). |
| `verifyTimeZone` | string | `"Etc/UTC"` | IANA timezone for `verifySchedule`. |
| `maxStaleness` | duration | `48h` | Threshold past which `Restorable=False` is stamped when no recent verification has succeeded (Phase 2). |
| `suspend` | bool | `false` | Pauses verification CronJob without deleting the CR. |
| `retentionFloorRefresh` | duration | `5m` | How often `dr-cursors/<name>.json` is refreshed (Phase 2). |

### StandbyActivateSpec

| Field | Type | Required | Default | Description |
|---|---|---|---|---|
| `confirm` | string | Yes | -- | RFC 3339 anti-fat-finger token. Controller activates only when this parses and is strictly greater than `status.activation.confirmTokenUsed` (Phase 3). |
| `pointInTime` | PointInTimeSpec | No | -- | Optional PITR stop time for binlog replay during activation. Mirrors `spec.initFromBackup.pointInTime` (Phase 3). |
| `acceptUnverified` | bool | No | `false` | Bypass `Restorable=True` gate. Use only when a stale archive is better than no archive (Phase 3). |
| `restoreTimeout` | duration | No | `2h` | Wall-clock cap on the materialized MFG reaching `Succeeded` before stamping `Failed` (Phase 3). |
...
|---|---|---|
| `BucketReadable` | Phase 1 | Bucket scan succeeded within the last discovery interval. Reason: `ListSucceeded` (True), `ListFailed` (False), `AuthFailed` (False — store construction failed, e.g. bad credentials), or `ConfigError` (False — missing/invalid spec fields). |
| `SourceConfigKnown` | Phase 1 | At least one full dump and one binlog manifest were found. Reason: `DumpFound` (True), `NoDumpFound` (False), `NoBinlogManifests` (False — dump present but no binlog manifests under `<prefix>/binlogs/`), `MetadataUnreadable` (False — dump `@.json` present but cannot be parsed), or `ConfigError` (False — propagated from storage backend error). |
| `Restorable` | Phase 2 | Most recent owned `MysqlBackupVerification` is Succeeded and within `freshness.maxStaleness`. |
