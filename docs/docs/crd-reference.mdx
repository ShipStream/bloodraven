...
| `recoveryState` | string | Old-primary recovery state: empty (no recovery needed) or `RecoveryBlocked` (divergent transactions detected — site must be wiped and re-cloned) |
