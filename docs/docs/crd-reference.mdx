...
| `recoveryState` | string | Old-primary recovery state: empty (no recovery needed) or `RecoveryBlocked` (divergent transactions detected — trigger a reclone to recover) |
...
| `RecoveryPending` | `True` when an old primary has divergent transactions and requires manual intervention. Reason is `DivergentTransactions`. Use the reclone annotation to recover. |
