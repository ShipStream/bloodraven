...
| `az` | string | Yes | -- | Availability zone identifier for DNS record naming |
| `cloudflare` | CloudflareSpec | Yes | -- | Cloudflare DNS configuration for traffic steering |
...
| `apiTokenSecretRef` | SecretKeyRef | Yes | Reference to the Secret containing the Cloudflare API token |
