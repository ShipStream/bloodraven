...
|---|---|---|---|
| `dataFileDir` | string | `/run/mysql-keyring` | Keyring mount point. Rejected if inside `/var/lib/mysql`; MySQL documents that the keyring data file must not be in the data directory, and co-locating them would put the key on the same stolen volume as the ciphertext. |
