...
|---|---|
| `bloodraven.shipstream.io/reclone-site` | Set to a site name (e.g. `iad`) to trigger a `CLONE INSTANCE` from the current primary to that site. The operator processes the annotation, removes it automatically, and reports progress via the `Bootstrapping` condition. Use this to recover a divergent site without manually deleting its PVC. |
