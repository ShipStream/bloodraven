...
- **Self-fences** if the operator AND every peer are unreachable: sets `SET GLOBAL super_read_only=ON` on its local MySQL instance to prevent a stale primary from accepting writes during a full isolation. A single reachable peer is enough to keep the primary writable.
