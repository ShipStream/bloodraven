...
That is the whole minimal configuration. Everything below is about what
it actually protects, what it does not, and how the keyring lifecycle
works — all of which you should read before turning it on in production.
    ...
    keyring-init ──seed──► /run/mysql-keyring/keyring  (tmpfs, rw)
                    ...
                    mysqld writes ────┤
                          ...
                          sidecar reads, hashes
                                      │
                     POST /keyring/escrow  (bearer token)
                                      ▼
...
| Metric | Meaning |
|---|---|
| `bloodraven_keyring_phase{phase}` | One-hot per site. **Alert on `phase="sealed"` being 0 for more than a few minutes.** |
| `bloodraven_keyring_escrow_version` | Current escrow version per site. |
...
| `bloodraven_encryption_unencrypted_tablespaces` | User tablespaces still reporting `ENCRYPTION='N'`. **Non-zero means part of your data is in the clear.** |
| `bloodraven_encryption_coverage{aspect}` | Per-aspect coverage (`system_tablespace`, `redo_log`, `undo_log`, `binlog`, `keyring_read_only`). |
...
by rolling a fresh escrow: re-render the site unsealed so the sidecar
re-escrows what MySQL is holding.

```bash
kubectl patch mysqlfailovergroup orders --subresource=status --type=merge -p \
  '{"status":{"encryptionAtRest":{"sites":[{"name":"iad","phase":"Unsealed","unsealReason":"Rotation"}]}}}'
```

If the pod is already gone, that site's data is unrecoverable — re-clone
it from a healthy peer.
...
| `keyring.dataFileDir` | `/run/mysql-keyring` | Keyring mount point. Must not be inside `/var/lib/mysql`. |
