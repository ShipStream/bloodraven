...
- **Image-specific paths.** MySQL only reads its global component
  manifest from the directory holding the `mysqld` binary, and its global
  component config from `plugin_dir`. Defaults match the official
  `mysql:9.x` images. If you run a different image, check them:
