...
2. Update the Secret referenced by `spec.cloudflare.apiTokenSecretRef`
3. The operator will use the new token on the next DNS operation
...
2. DNS records are cleaned up
3. MySQL pods, Services, and PVCs are deleted
