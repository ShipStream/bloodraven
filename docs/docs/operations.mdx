...
| Healthy | `Bootstrapping=False`, `RecoveryPending=False`, target `state=read-only`, replication running, and `divergentGtid` empty. |
...
See the [Upgrade and version-skew policy](./upgrade-policy.mdx) for which MySQL tags are supported, how the 9.6 → 9.7 transition works, and the cross-site and sidecar ↔ operator skew contract.
