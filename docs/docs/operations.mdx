...
To recover the site, wipe its data and let the operator re-clone from the current primary:
...
**Step 3: Delete the pod to trigger recreation**

```bash
kubectl delete pod mysql-orders-iad-0
```

The operator will detect the fresh MySQL instance and bootstrap it as a replica via `CLONE INSTANCE`, restoring it to a consistent state with the current primary.
