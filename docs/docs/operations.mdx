...
Use the reclone annotation to have the operator run `CLONE INSTANCE` on the divergent site, replacing all its data with a fresh copy from the current primary:

```bash
kubectl annotate mysqlfailovergroup orders bloodraven.shipstream.io/reclone-site=iad
```

The operator processes the annotation, removes it automatically, and begins cloning. The site will temporarily appear unreachable during the MySQL restart that follows the clone. Progress is reported in the `Bootstrapping` condition.
