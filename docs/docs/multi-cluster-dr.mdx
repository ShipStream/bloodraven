...
```bash
curl -f http://<source-operator-svc>:8080/active-site
# Healthy: {"activeSite":"iad"}
# Down:    curl: (22) The requested URL returned error: 503
```

The operator's auxiliary HTTP server (`cmd/bloodraven/main.go:361`)
returns 503 when it cannot reach any writable site.
...
spec:
  ...
  source:
    ...
    decryption:
      passphraseSecret:
        name: orders-backup-passphrase
  # template and freshness/activate fields exist in the CRD but are
  # consumed by Phase 2 (Restorable verification) and Phase 3 (activation).
  # Phase 1 reads only source.storage to drive the discovery loop.
```
