...
| `KeyringRecovered` | Normal | A previously `Failed` sealed site is back to `Sealed`: its escrow Secret is present and matches the recorded digest, and MySQL confirms it is running that same keyring. |
