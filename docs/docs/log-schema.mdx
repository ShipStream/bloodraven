...
| WARN | `dragonfly: stale master on non-active site` | `site`, `active`, `fg` | A site reports `role=master` but is not the active site. Operator does not auto-fence in this version (see wishlist 44); apps continue routing to the active master via the label selector. |
...
| `DragonflyStaleMasterDetected` | A non-active site reports master role. Logged + dedup'd in 5-minute windows. |
