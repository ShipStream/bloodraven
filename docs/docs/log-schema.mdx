...
| WARN | `SELF-FENCING: failed to kill connections after fencing` | `error`, `count`, `pod` | The fence write landed but eviction was incomplete. `count` is how many sessions were killed — `0` when the process-list query itself failed, so no session was ever enumerated. `error` joins every cause that applied: the listing query failed, iteration ended early, some rows would not scan, or some `KILL`s were refused. A `KILL` for a session that had already exited is not a cause. `super_read_only=ON` still holds, so surviving sessions cannot write — this is degraded, not unsafe. |
| WARN | `SELF-FENCING: giving up on evicting connections after fencing` | `error`, `count`, `attempts`, `pod` | An incomplete eviction was retried on later ticks and never completed, so the sidecar stopped trying. Sessions that survive this are refusing to die for a structural reason (a missing privilege, a session the server will not let the sidecar touch). `super_read_only=ON` still holds; expect stale reads from those sessions, not writes. |
| INFO | `SELF-FENCING: abandoning eviction retry, fence already lifted` | `pod` | A pending eviction retry found `super_read_only` already off, meaning the operator is restoring or promoting this site. The retry is dropped rather than run — promotion clears `super_read_only` before `read_only`, and evicting inside that window would kill the operator's own promotion session. |
| WARN | `fencing: could not check super_read_only before eviction retry` | `error`, `pod` | The retry could not confirm the fence still holds, so it was skipped for this tick and stays armed for the next. |
