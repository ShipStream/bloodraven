...
| WARN | `SELF-FENCING: failed to kill connections after fencing` | `error`, `count`, `pod` | The fence write landed but eviction was partial: `count` sessions were killed and `error` names the gap (an unreadable processlist row, or an iteration that ended early). `super_read_only=ON` still holds, so this is degraded, not unsafe. |
