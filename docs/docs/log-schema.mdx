...
| WARN | `SELF-FENCING: failed to kill connections after fencing` | `error`, `count`, `pod` | The fence write landed but eviction was incomplete. `count` is how many sessions were killed — `0` when the process-list query itself failed, so no session was ever enumerated. `error` joins every cause that applied: the listing query failed, iteration ended early, some rows would not scan, some `KILL`s were refused, or the fence hit its 20s deadline (the whole sequence is bounded so a hung write or `KILL` cannot stall the monitor's tick loop and stop later fencing checks). A `KILL` for a session that had already exited is not a cause. `super_read_only=ON` still holds, so surviving sessions cannot write — this is degraded, not unsafe. The sidecar does not retry. A **planned failover** drains stragglers itself (`Draining` kills every second until zero or `spec.plannedFailover.drainTimeout`), but an autonomous self-fence — topology mismatch or lease expiry — has no such follow-up, so a surviving session can stay connected for stale reads until the site is next promoted or demoted. If this fires outside a planned failover, treat it as actionable: check the joined causes for a permission problem, and `KILL` the remaining sessions by hand if they matter. The sidecar's `/status` reports `self_fenced` so you can tell which sites fenced themselves without reading logs — scoped to the current sidecar process, so a container restart clears it while `super_read_only` stays on. |
| WARN | `SELF-FENCING: super_read_only write failed but the fence is in place; skipping connection eviction` | `error`, `pod` | The `SET GLOBAL` returned an error — commonly the 20s fence deadline, or a connection lost while waiting for the reply — but a follow-up read shows the instance read-only, so the write had already landed. The fence counts as established: `/status` reports `self_fenced=true` and `SELF-FENCED` follows. Eviction is skipped because the fence budget is spent, so surviving sessions can hold stale reads exactly as in the incomplete-eviction case above. |
| ERROR | `SELF-FENCING FAILED: could not set super_read_only` | `error`, `pod` | The fence write failed and a follow-up read either found the instance still writable or could not be answered. The sidecar retries on the next tick. |
| WARN | `fencing: could not confirm whether the super_read_only write landed` | `error`, `pod` | Emitted alongside the line above when the follow-up read itself failed, so the fence outcome is unknown. Treated as not fenced and retried next tick. |
...
| ERROR | `out-of-band anti-flap state write failed; retrying every poll` | `target`, `lastFailover`, `error` |
