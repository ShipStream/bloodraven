...
2. If it is running, re-escrow by re-rendering the site unsealed. The
   sidecar pushes whatever MySQL is holding, and the operator mints a new
   version:

   ```bash
   kubectl patch mysqlfailovergroup orders -n orders --subresource=status --type=merge -p \
     '{"status":{"encryptionAtRest":{"sites":[{"name":"iad","phase":"Unsealed","unsealReason":"Rotation"}]}}}'
   ```

   Watch it return to `Sealed` with a new `keyringVersion`.
3. If the pod is already gone, that site's data is unrecoverable.
   Re-clone it from a healthy peer:
