...
<img src={require('/static/img/infographics/runbooks.webp').default} alt="runbooks infographic" />
...
Full background: [Data-at-rest encryption](./encryption-at-rest.mdx).

## Keyring escrow lost

Use on a `KeyringEscrowMissing` or `KeyringEscrowCorrupt` Event.

:::danger Do not restart the affected pod
...
2. If it is running, recreate the exact missing Secret from the keyring
   ...
   Do **not** force the phase back to `Sealed` by patching status. The
   operator re-runs its own drift check on the next reconcile: once the
   Secret is present, hashes to the recorded digest, and MySQL confirms
   it is running that same keyring, the site returns to `Sealed` on its
   own and emits `KeyringRecovered`. Forcing the phase would skip exactly
   the confirmation that makes the restore safe.

   ```bash
   kubectl get mysqlfailovergroup orders -n orders \
     -o jsonpath='{.status.encryptionAtRest.sites[?(@.name=="iad")].phase}{"\n"}'
   ```

   Verify that the site returns to `Sealed` without restarting the pod.
   If it stays `Failed` with a message about waiting for MySQL to confirm
   the keyring, the bytes you restored are not the bytes the running
   instance is using — do not restart the pod, and treat the site as
   re-clone-only.
