...
|---|---|
| Prometheus metrics | [Monitoring Reference](./monitoring.mdx) |
| Recording rules | [Monitoring Reference](./monitoring.mdx) or the rule package docs |
| Alerts | [Alert To Runbook Map](./alert-runbook-map.mdx) and the alert package docs |
| Dashboard panels | [Grafana Dashboards](./monitoring-grafana.mdx) and shipped JSON under `charts/bloodraven/dashboards/` |
| Kubernetes Events | [Monitoring Reference](./monitoring.mdx) for the Event reason registry; [Alert To Runbook Map](./alert-runbook-map.mdx) when the Event needs operator action |
| Structured-log Events | [Log Schema](./log-schema.mdx) |
| Runbook links | [Runbooks](./runbooks.mdx), page-specific runbooks, and [Alert To Runbook Map](./alert-runbook-map.mdx) |
...
- Runbook mapping in [Alert To Runbook Map](./alert-runbook-map.mdx), or a specific no-runbook rationale.
...
Structured-log Events are stable log `msg` strings and fields consumed by downstream log pipelines. They are documented in [Log Schema](./log-schema.mdx) and are a public stability contract.
...
- Any matching update to [Log Schema](./log-schema.mdx).
