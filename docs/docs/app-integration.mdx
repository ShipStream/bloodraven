...
Best for applications that run on **dedicated nodes per site** and should be fully migrated when the database fails over.

**How it works:**

1. The operator taints nodes at the old active site with `shipstream.io/db-readonly=true:NoExecute`
...
- Do **not** add a toleration for `shipstream.io/db-readonly:NoExecute`
  ...
  containers:
    - name: app
      ...
      env:
        ...
        - name: MYSQL_PORT
          value: "3306"
  # No toleration for shipstream.io/db-readonly -- pod will be evicted on failover
```
...
- **Not** cache DNS indefinitely -- respect TTLs so that Cloudflare DNS changes take effect

## External access via Cloudflare DNS

For applications outside the Kubernetes cluster, the operator manages Cloudflare DNS A-records that point to the active site's load balancer IP (`spec.sites[].lbIP`). When a failover occurs, the operator flips the DNS record to the new active site.

External applications should:

- Use the DNS hostname configured in Cloudflare (not hardcoded IPs)
- Respect DNS TTLs
