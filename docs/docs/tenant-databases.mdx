...
`spec.owner.privileges` is desired state in both directions: the reconciler revokes the owner's existing grant on the database before granting the declared set, so narrowing `[ALL PRIVILEGES]` to `[SELECT]` actually narrows it. (This applies to the CR-owned owner only — `grants[]` principals are shared, see Known gaps.)
...
- **Removing a `grants[]` entry does not revoke it.** The grant stays in place until the CR is deleted under `deletionPolicy: Delete`. Diffing against `status.appliedGrants` and revoking is probably the right behaviour, but it makes a CR edit destructive, so it is deliberately not implemented yet. (The owner's own privileges do not have this gap — they are revoked-and-regranted on every apply.)
