...
**`status` never carries credential material.** `ownerUser` is a username. `lastAppliedHash` is a digest of the Secret's bytes, not the bytes.
...
| `Failed` | Invalid identifier, a `grants[]` user that does not exist, an ownership conflict with another CR, a reserved owner username, or a MySQL verdict about the CR's own statements. |
...
Identifiers are validated before they reach SQL rendering, not merely escaped: `databaseName`, `characterSet` and `collation` must match `^[A-Za-z0-9_]{1,64}$`, and usernames `^[A-Za-z0-9_][A-Za-z0-9_.$-]{0,31}$`. Rejection is the contract; escaping is the belt on top of the braces.
...
`spec.databaseName` is also **immutable**, enforced by the API server. MySQL has no schema rename; an edited name would CREATE a second database and orphan the first. Renaming a tenant database is a migration, not a spec edit.
...
The reconciler watches the referenced Secret, so it applies `ALTER USER` on the next reconcile. Re-applying an unchanged CR issues **zero** MySQL statements — the reconciler compares a hash of the spec, the Secret's contents and the active site against `status.lastAppliedHash` — which is what keeps a tenant-dense cluster from hammering the primary.

Rotating the **username** in the Secret is also just a Secret write, and it revokes what it replaces: the reconciler creates the new owner, grants it, and drops the previous owner account (`OwnerUserRotated` event). A rotation performed because a credential leaked actually revokes the leaked credential — `status.ownerUser` keeps the old name until the old account is really gone, so a failure mid-rotation retries rather than leaving a shadow account.

Privileges are desired state in both directions, for the owner and for every `grants[]` entry still listed in the spec: the reconciler revokes the user's existing grant on this database before granting the declared set, so narrowing `[ALL PRIVILEGES]` to `[SELECT]` actually narrows it. The revoke is scoped to this database, and the one-database-one-CR rule (above) is what makes it safe — no other CR can be managing the same grant. Entries *removed* from `grants[]` are the remaining gap, see Known gaps.
...
| `Delete` | Revoke the `grants[]` entries on this database, `DROP DATABASE`, `DROP USER` the owner, then remove the finalizer. Never drops a `grants[]` user — those principals are shared. |
...
- `status.databaseCreated` is a write-ahead record, stamped before the first statement ever executes. A CR that failed before any SQL ran (invalid spec, reserved owner, ownership conflict) releases with `DatabaseDropSkipped` and touches nothing — it must not drop a database something else created under the same name. A CR that failed *mid*-apply is covered in the other direction: its owner user is recorded and gets dropped rather than surviving as an orphaned privileged account.
- If another live CR on the group still declares the same `databaseName` or shares the owner principal, the drop is scoped down to what is exclusively this CR's, with a `DatabaseDropSkipped` warning for the rest.
...
Under `Delete`, if the group has no active site — or its primary is fenced by an in-place restore or planned failover — the drop is **deferred**, not skipped: the CR emits `DatabaseDropDeferred` and waits. If the group is gone entirely there is nothing to connect to, so the finalizer is released with a `DatabaseCleanupSkipped` warning rather than wedging the CR forever. While the finalizer runs, `status.phase` is `Deleting`.
...
Both paths connect to the same primary through the same function — `openAdminConnection` in `internal/controller/credentials.go` — and manage disjoint principals. That function is the only place in Bloodraven that assembles MySQL admin credentials, and it has exactly these two callers. A third would be a design decision, not a refactor.

## Known gaps

- **Removing a `grants[]` entry does not revoke it.** The grant stays in place until the CR is deleted under `deletionPolicy: Delete`. Diffing against `status.appliedGrants` and revoking is probably the right behaviour, but it makes a CR edit destructive, so it is deliberately not implemented yet. (Entries still *listed* do not have this gap — the owner and every listed grant are revoked-and-regranted on each apply, so edits converge.)
