...
All three fields are optional; omitting the block is equivalent to `{maxLagWait: 5m, drainTimeout: 30s, onCooldown: reject}`.
...
`PlannedFailoverStarted`, `PlannedFailoverSkipped` (already-active target), `PlannedFailoverRejected`, `PlannedFailoverDeferred` (cooldown + defer policy), `PlannedFailoverDraining`, `PlannedFailoverLagOK`, `PlannedFailoverCompleted`, `PlannedFailoverFailed`.
