...
- [x] 36. Move tenant Dragonfly ownership from the platform tenant chart into Bloodraven. _(Bloodraven now reconciles the managed Dragonfly StatefulSets, site-local Services, active Service, auth, resources, placement, status, promotion, and cleanup from `spec.dragonfly`; the playground and Helm RBAC/manifests exercise the Bloodraven-owned path.)_
...
- [x] 38. Reconcile per-site Dragonfly workloads, Services, auth, PDBs, and placement. _(per-site StatefulSets/Services, active Service, group-level `minAvailable: 1` PDB, auth, resources, and placement are reconciled from `spec.dragonfly`)_
...
- [x] 43. Add metrics, Events, dashboards, and runbook docs for Dragonfly state. _(Dragonfly metrics and Events are registered; user-facing monitoring/playground docs now document the supported image floor, ephemeral-data contract, unsafe args policy, topology-change/full-resync cost, and operator metrics to alert on. The metrics are available for dashboards; dedicated Dragonfly-only panels can be added later without blocking this slice.)_
- [x] 44. Add k3d E2E and chaos scenarios covering MySQL + Dragonfly + DNS failover. _(playground wiring done: `spec.dragonfly` in playground FG, setup.sh waits for StatefulSets, `chaos.sh kill-dragonfly|dragonfly-status`, counter-app demonstrates cache continuity via Dragonfly INCR. Slice 3: unit-level no-`READONLY`-mid-flight regression delivered via `TestPlannedFailoverPromotingDragonfly_NoReadOnlyMidFlight` (asserts the active-Service selector invariant by intercepting every Pod label patch and tracking the Dragonfly master pointer through `ReplTakeover`). The live k3d `chaos-run-all` suite passed on 2026-05-03, covering the MySQL failover, planned switchover, Dragonfly promotion, active Service endpoint, DNS flip, reset, and snapshot-upgrade paths.)_
- [x] 45. Integrate Dragonfly into the chaos test harness from PR #68 (`cmd/playground-chaos`).
  ...
  - **Baseline assumptions**: `playground/manifests/failovergroup.yaml` has `spec.dragonfly.enabled: true` with `maxSyncWait: 30s`, `onSyncTimeout: proceed`, image `docker.dragonflydb.io/dragonflydb/dragonfly:v1.38.0`, ports 6379/9999. `AssertHealthyBaseline` (`internal/playground/scenarios/common.go`) requires `status.dragonfly.phase=Ready`, exactly one master, every other site `role=replica + linkStatus=up + lastIOSecondsAgo>=0 + !syncInProgress`, no `unreachable` sites — fix any drift before scenarios will pre-pass.
  - [x] Harness primitives shipped (commit `11cf391`)
    ...
    - `internal/playground/dragonfly/{site,resp,credentials}.go` — port-forwarded RESP client. `Open(ctx, k, ns, fg, site, password)` returns a `SiteClient` with `Ping`/`Set`/`Get`/`Incr`/`DBSize`/`InfoReplication`. `IsReadOnlyError` and `IsConnDropped` are the discriminators chaos scenarios pattern-match on (READONLY = real bug; conn-dropped = expected after CLIENT KILL). `LoadPassword` now decodes `spec.dragonfly.auth.SecretName` / `passwordKey` and the runner passes it into every chaos Dragonfly client.
  ...
  - [x] Runner cleanup waits for Dragonfly reconvergence after each scenario (`waitForClusterReconverge` now requires `status.dragonfly.phase=Ready`, exactly one master, and healthy replicas when Dragonfly is enabled). This fixes the `run-all` cascade where scenario 01 could finish MySQL cleanup while the old Dragonfly master was still recovering from REPLTAKEOVER, causing every later scenario to fail precheck on `status.dragonfly.phase=Degraded`.
  ...
  - [x] Scenario 22 passes against live k3d (2026-05-03): `PASS 22-planned-dragonfly-switchover`, duration 4.255s. Verified `PromotionMethod=REPLTAKEOVER`, `SessionsPreserved=true`, seed key readable on the new master, active Service endpoints converged, and success metric incremented.
  - [x] Scenario 23 passes against live k3d (2026-05-03): `PASS 23-dragonfly-master-kill`, duration 7.236s. Fixed two issues found by the live run: Dragonfly-only emergency promotion is now driven by `DragonflyManager` when the current cache master is unreachable, and the scenario chooses the peer of `status.dragonfly.activeSite` rather than the peer of MySQL `status.activeSite` after a legitimate Dragonfly/MySQL divergence.
  - [x] Scenario 24 passes against live k3d (2026-05-03): `PASS 24-emergency-mysql-dragonfly-down`, duration 48.220s. MySQL emergency failover completed while every Dragonfly StatefulSet was scaled to 0; observed latency included the expected dead-primary relay-log drain.
  - [x] D1 / D2 coverage — intentionally folded into the Dragonfly baseline and cleanup gates rather than standalone scenarios. Every Dragonfly-enabled scenario now fails precheck/cleanup if replicas are loading, never synced, link-down, or not Ready; scenarios 22, 23, 25, 26, 27, and 29 additionally force promotion/restart paths that exercise the same contracts in live k3d. Standalone D1/D2 files can still be added later if these diagnostics become too coarse, but they are no longer required for this production-readiness slice.
  - [x] Scenario 26 registered (D4 — sync timeout with `onSyncTimeout=proceed`): `internal/playground/scenarios/s26_planned_dragonfly_sync_timeout.go`. New chaos action `Actions.PatchDragonflySyncBudget` (JSON Patch on `spec.dragonfly.plannedFailover.{maxSyncWait,onSyncTimeout}` with capture-original-then-restore reverter). Inject sequence: tighten budget to `1ms`, scale target Dragonfly StatefulSet to 0, annotate planned-failover. Asserts (a) `plannedFailover.phase=Succeeded`, (b) MySQL `status.activeSite` flipped (Dragonfly degradation never blocks MySQL), (c) `status.plannedFailover.dragonfly.SessionsPreserved=false` (the headline contract — a timeout MUST NOT masquerade as a clean session-preserving failover), (d) `Reason ∈ {DragonflySyncTimeout, DragonflyPromotionFailed}` (the two documented timeout/failure code paths), (e) `bloodraven_dragonfly_promotions_total{result="failed"}>=1`. Settles by force-respawning the source's Dragonfly pod so its stale-master state (master_repl_offset>0 = below the auto-rejoin gate) is wiped and the next baseline holds.
  - [x] D6a (snapshot-restore planned upgrade; short outage accepted) — first-class planned-maintenance path for rare Dragonfly upgrades where cache/session outage is acceptable. Contract: admin requests a target image explicitly, Bloodraven strips active Dragonfly traffic, issues `SAVE` against the current active master, updates/restarts the active pod with `spec.dragonfly.snapshot.dir` (`s3://...`) configured, waits for that pod to restore and serve as master on the new image, then updates and reattaches replica sites. This is the "poor man's rolling upgrade" path and is intentionally separate from ordinary `spec.dragonfly.image` patching so a casual spec edit cannot surprise-drop cache traffic.
    - [x] API/resource groundwork: `spec.dragonfly.snapshot.dir` renders as Dragonfly `--dir`, `spec.dragonfly.snapshot.serviceAccountName` assigns an IAM-capable ServiceAccount to Dragonfly pods, user `--dir` args are filtered as operator-owned, and the Dragonfly client has a `SAVE` primitive.
    - [x] One-shot D6a state machine: accepts `bloodraven.shipstream.io/dragonfly-snapshot-upgrade=<target-image>`, persists `status.dragonfly.upgrade`, pauses autonomous Dragonfly replication steering, sheds active Service traffic, `SAVE`s, updates/restarts active StatefulSet, waits for restore, updates replicas, reattaches replicas with `REPLICAOF`, restores traffic, clears annotation, and stamps Succeeded/Failed with Events.
    - [x] D6a chaos scenario hardening: `29-dragonfly-snapshot-upgrade` now self-provisions the playground RustFS `dragonfly` bucket through a minimal in-runner SigV4 S3 client, temporarily patches `spec.dragonfly.snapshot` for the scenario, verifies pods restart with `--dir=s3://dragonfly/playground`, then requests the upgrade. Live k3d (2026-05-03): `PASS 29-dragonfly-snapshot-upgrade`, duration 4m26.292s. The live run also found and fixed missing pod `delete` RBAC needed for the active-pod restart step.
  - [x] D6b (true rolling Dragonfly upgrade; no global outage) — `reconcileDragonflyResources` now serializes template drift: update one non-active StatefulSet, wait for rollout, promote that updated replica with REPLTAKEOVER, then roll the old active site. The drift comparison ignores Kubernetes defaulted fields and compares only Bloodraven-owned template fields so an already-updated replica does not look stale forever. Scenario `27-dragonfly-rolling-image-update` patches `spec.dragonfly.image` to the cached digest reference reported by the running pod, asserts no point has both Dragonfly pods unavailable, and passed live k3d (2026-05-03): `PASS 27-dragonfly-rolling-image-update`, duration 16.159s.
  - [x] D11 (operator restart mid-Dragonfly-failover) scenario registered and live-passing — `internal/playground/scenarios/s25_operator_restart_mid_dragonfly_failover.go`. The scenario now creates a deterministic `WaitingForDragonflySync` window by temporarily scaling the target Dragonfly StatefulSet to 0 with a 45s sync budget, kills the operator after observing the fresh in-flight phase (stale `plannedFailover` statuses are ignored by `StartTime`), restores Dragonfly, and verifies the resumed operator reaches `Succeeded` with both MySQL and Dragonfly on the original target. Live k3d: `PASS 25-operator-restart-mid-dragonfly-failover`, duration 53.802s.
  - [x] Scenario 26 passes against live k3d (2026-05-03): `PASS 26-planned-dragonfly-sync-timeout-proceed`, duration 11.273s. The failed-promotion path stamped `SessionsPreserved=false`, moved MySQL to the target anyway, incremented the failed metric, and the extended settle step force-respawned the old Dragonfly source back as a replica.
  - [x] D3/D4/D5/D7/D11 live outcomes folded into `playground/chaos-scenarios.md` and this plan.
...
## Required Before Next Slice (closed)

The no-`READONLY` mid-flight regression is now covered at two levels. `TestPlannedFailoverPromotingDragonfly_NoReadOnlyMidFlight` intercepts every pod label patch during `PromotingDragonfly` and asserts the active Service selector never exposes a stale read-only master while the Dragonfly master pointer moves through `REPLTAKEOVER`. Live k3d scenario 22 then verifies the real endpoint-controller side of that contract by asserting the app-facing Dragonfly Service endpoints converge to the promoted master after planned switchover; the full `chaos-run-all` suite passed with that coverage on 2026-05-03.
