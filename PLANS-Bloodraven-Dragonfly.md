...
- [ ] 45. Integrate Dragonfly into the chaos test harness from PR #68 (`cmd/playground-chaos`).
  ...
  - [ ] D11 (operator restart mid-Dragonfly-failover) scenario — kill operator after `phase==WaitingForDragonflySync` lands but before `Promoting`, restart, assert convergence. Reuses `KillOperator` / `ScaleOperatorToZero` from existing chaos actions; tricky part is the timing window (need a tight log-tail on `dragonfly: REPLTAKEOVER` and a quick `Chaos.KillOperator` racing it).
