...
- [ ] 45. Integrate Dragonfly into the chaos test harness from PR #68 (`cmd/playground-chaos`).
  ...
  - [ ] D6a (snapshot-restore planned upgrade; short outage accepted) — first-class planned-maintenance path for rare Dragonfly upgrades where cache/session outage is acceptable. Contract: admin requests a target image explicitly, Bloodraven strips active Dragonfly traffic, issues `SAVE` against the current active master, updates/restarts the active pod with `spec.dragonfly.snapshot.dir` (`s3://...`) configured, waits for that pod to restore and serve as master on the new image, then updates and reattaches replica sites. This is the "poor man's rolling upgrade" path and is intentionally separate from ordinary `spec.dragonfly.image` patching so a casual spec edit cannot surprise-drop cache traffic.
    ...
    - [x] D6a chaos scenario: playground now deploys RustFS (`rustfs/rustfs:latest`) plus a bucket-init Job, configures Dragonfly snapshots to `s3://dragonfly/playground` with `--s3_endpoint=rustfs... --s3_use_https=false`, and registers `29-dragonfly-snapshot-upgrade` to seed Dragonfly, request snapshot-restore upgrade, assert `status.dragonfly.upgrade=Succeeded`, verify the seed key restored, and confirm all Dragonfly StatefulSets use the target image.
  - [ ] D6b (true rolling Dragonfly upgrade; no global outage) — patch `mfg.Spec.Dragonfly.Image`, observe one-pod-at-a-time rollout, assert no window where both pods are simultaneously NotReady. **Blocked**: requires operator-side serialized cross-site Dragonfly rollout (replica-first-then-master-with-promotion) per the regression-guard description in `PLANS-Dragonfly-Chaos-Scenarios.md` D6 — the current `reconcileDragonflyResources` updates every site's StatefulSet in a single sequential pass with no readiness gate between sites. Implement that coordination path before adding the scenario, otherwise the test would pin failing behavior. Action helper still applies (`pgchaos.PatchDragonflyImage(image)` with reverter).
  ...
  - [ ] Scenario 26 passes against live k3d
    - Watch for: a Reason of `DragonflySyncTimeout` AND a Reason of `DragonflyPromotionFailed` are both legal end-states. The handler that fires depends on a 1-2 reconcile-tick race in WaitingForDragonflySync — either branch arrives at SessionsPreserved=false, which is what the assertion gates on. If a third Reason value appears, a new untested code path is now reachable; investigate before relaxing the assertion.
    - Likely flake source: the settle step's source-respawn wait depends on the auto-rejoin gate (`connected_slaves=0 AND master_repl_offset=0`). If a stray write reaches the fresh source pod between respawn and the DragonflyManager tick, the gate flips closed and the wait times out. Increase the budget only after confirming the operator log shows REPLICAOF-attempt / no-attempt; raising it blindly can mask a real regression.
  ...
  - [ ] After D3/D5/D7 soak, fold the live-cluster outcomes back into `playground/chaos-scenarios.md` and into `PLANS-Dragonfly-Chaos-Scenarios.md`. Implementation docs are now folded in and D3/D4/D5/D7/D11 are marked runnable; live pass/fail outcomes are still pending actual k3d runs.
