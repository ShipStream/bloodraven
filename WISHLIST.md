...
**8. DR drill automation / backup verification.** Ship a `MysqlBackupVerification` CRD or CronJob template that periodically restores the latest backup into a throwaway namespace, runs a sanity query, and emits a `bloodraven_backup_verified_timestamp_seconds` gauge. Unverified backups are schrödinger backups.
