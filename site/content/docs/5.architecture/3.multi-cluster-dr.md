---
title: Multi-cluster disaster recovery
---
...
- `dr-only` site roles (`api/v1alpha1/types.go:280-283`,
  [Multi-site topology](/docs/architecture/multi-site)) are a separate concept: they
...
**Step 1.** Copy the passphrase from the source cluster into a temporary file on a trusted workstation:

```bash
PASSFILE=$(mktemp)
kubectl -n orders get secret orders-backup-passphrase \
  -o jsonpath='{.data.passphrase}' | base64 -d > "$PASSFILE"
```

**Step 2.** Point `kubectl` at the DR cluster and create the same Secret in the DR namespace from that file:

```bash
kubectl -n orders create secret generic orders-backup-passphrase \
  --from-file=passphrase="$PASSFILE"
shred -u "$PASSFILE"
```

**Step 3.** Reference it in the DR restore manifest via
`spec.initFromBackup.decryption.passphraseSecret.name`. See
[Backup encryption](/docs/backup-and-restore/backup-encryption) for full field semantics.
...
[Durability and RPO](/docs/architecture/durability-and-rpo#the-exact-bound-divergentgtid)).
...
- The DR cluster has a fresh namespace `orders` with Bloodraven
  ...
  [Install production](/docs/get-started/install-production)).
...
(see [PITR and the RPO window](/docs/architecture/durability-and-rpo#pitr-and-the-rpo-window)).
...
- One-command activation (`dr-activate` kubectl plugin + a future
  activate-request state machine) — Phase 3.
...
- **Phase 3** — One-command activation: `kubectl bloodraven dr-activate`
  ...
  `MysqlFailoverGroup` from the discovered dump. The activation state
  machine is phase-stamped and durable across operator restarts. The spec
  field that carries the confirm token will be added when this lands.
