...
Limits are desired state in both directions: omitting one renders `0` — MySQL's "unlimited" — on the next apply, so removing a limit from the spec actually clears it rather than silently leaving the old cap enforced.
...
|---|---|
| `Retain` (default) | Remove the finalizer, leave MySQL untouched, emit `DatabaseRetained`. No connection is opened — `users[]` principals survive too. |
