...
The optional `spec.deploymentClients` list authorizes projected ServiceAccounts to use the group's [Deployment API](/docs/configuration/deployment-api). It does not grant MySQL privileges. Each entry contains `namespace` and `serviceAccount`; namespace is the list-map key, so one ServiceAccount can be declared per namespace. An omitted or empty list authorizes no deployment callers.
...
Bloodraven only manages what it created. If the schema named by `databaseName` already exists on the group and this CR's `status.databaseCreated` does not say it created it, the CR fails with `reason: DatabasePreExists` and runs no SQL. Likewise, if the owner username already exists in MySQL and is not recorded as this CR's owner, the CR fails with `reason: PreExistingOwnerUser` before any `ALTER USER` resets a foreign account's password — and a `users[]` username that exists without a matching `status.appliedUsers` record fails the same way with `reason: PreExistingUser`. A `users[]` username that a live sibling CR on the group already claims — as its owner, in its `grants[]`, or in its own `status.appliedUsers` ledger — is refused with `reason: UserClaimedBySibling` before the CR connects to MySQL at all, whether or not the account exists yet. None of these refusals authorizes a later `deletionPolicy: Delete` to touch the foreign schema or account: every write-ahead record stamped ahead of the refused statement (`databaseCreated`, `ownerUser`, `pendingOwnerUser`, `ownerHosts`, the `appliedUsers` ledger) is rolled back to its pre-reconcile value, and the owner Secret is never a source of drop candidates — after a refused rotation it names somebody else's account. To hand an existing schema over to a `MysqlDatabase`, drop and recreate the CR only if you own the schema — otherwise pick a new `databaseName`.
