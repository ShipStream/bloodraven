...
- If you enable `spec.dragonfly`, pin `spec.dragonfly.image` to Dragonfly `v1.38.0+` and route applications to the Bloodraven-created active Dragonfly Service. Confirm `status.dragonfly.replTakeoverSupported=true` after the first probe — Bloodraven reports the capability; it does not reject an older tag.
