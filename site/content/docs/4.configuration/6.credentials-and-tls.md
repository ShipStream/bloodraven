...
Adopting `spec.tls`, changing `spec.tls.secretName`, or rotating the TLS Secret bytes restarts the topology manager so the operator's own MySQL checkers pick up the new material. MySQL pods roll independently via the spec-hash drift path.
