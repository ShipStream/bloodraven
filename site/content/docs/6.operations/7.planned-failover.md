...
Manual promotion via three `kubectl exec ... SET GLOBAL ...` commands (see [Operations](/docs/operations/operations#manual-promotion-fallback-operator-unreachable)) works but has four real problems:
...
| `Deferred` | Cooldown with `onCooldown: defer`, or an unexpired deployment `failover-hold` with reason `DeploymentHold`. Annotation is retained; reconciler re-validates at `status.plannedFailover.retryAfter`. |
...
A [deployment hold](/docs/configuration/deployment-api#durability-and-fencing) defers validation regardless of `onCooldown`. `reason: DeploymentHold` names the operation and instance in the message, and `retryAfter` is the earliest hold expiry. Renewal can extend the wait. Ordered update and restore-in-place honor the same holds, while emergency failover and fencing never consult them. A hold does not relax any normal preflight requirement when it expires. Use the [explicit revocation procedure](/docs/operations/operations#deployment-coordination), not Lease deletion, when approved maintenance must interrupt a deployment.
...
| Deployment `failover-hold` active | `phase: Deferred`, `reason: DeploymentHold`, message names operation and instance, `retryAfter` is earliest expiry | no fence applied; never delays emergency fencing or failover |
...
| Target is mid-keyring-rotation (`UnsealReason=Rotation`) | `phase: Failed`, `reason: KeyringRotation`, event `PlannedFailoverRejected` | no fence applied; wait for `Sealed` and re-annotate |
...
| `bloodraven_planned_failovers_deferred_total` | counter | `group`, `reason` (including `DeploymentHold`) |
| `bloodraven_dragonfly_promotions_total` | counter | `group`, `target_site`, `result` (`success`, `failed`, `skipped`, `sessions_lost`) |
...
`PlannedFailoverStarted`, `PlannedFailoverSkipped` (already-active target), `PlannedFailoverRejected`, `PlannedFailoverDeferred` (cooldown + defer policy, or deployment hold), `PlannedFailoverDraining`, `PlannedFailoverLagOK`, `PlannedFailoverCompleted`, `PlannedFailoverFailed`, `DragonflyPromotionStarted`, `DragonflyPromotionCompleted`, `DragonflyPromotionFailed`, and `DragonflySyncTimeout`. A topology change revokes deployment leases and emits `DeploymentLeaseRevoked`.

## RPO

Planned failover is the **one** Bloodraven operation with an RPO of zero by construction. The zero-lag gate at `WaitingForLag` guarantees the target has replicated every committed transaction on the fenced source before promotion. Any scenario that would produce loss (target falls behind, source crashes mid-drain, catch-up times out) routes to the `Failed` rollback path instead of promoting a lagging replica. See [Durability and RPO](/docs/architecture/durability-and-rpo).
