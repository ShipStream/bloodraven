---
title: How to create your first MysqlFailoverGroup
---

![getting started infographic](/img/infographics/getting-started.webp)
...
For a complete local demo with a dashboard, counter app, and chaos scenarios, use the [Playground](/docs/get-started/playground). For production dependency ownership, image pinning, monitoring, and NetworkPolicy, use [Production Install](/docs/get-started/install-production).
...
See [Placement Contract](/docs/configuration/placement-contract) for the full labeling and scheduling contract.
...
|---|---|---|
| MySQL pod remains Pending | Node labels or StorageClass do not match the site. | Check [Placement Contract](/docs/configuration/placement-contract). |
| Operator logs mention missing Secret keys | The credential Secret is missing a required key. | Recreate the Secret from [Credentials and TLS](/docs/configuration/credentials-and-tls). |
| DNS does not change | external-dns is not watching the namespace or `DNSEndpoint` CRD. | Check [Troubleshooting](/docs/operations/troubleshooting#dns-not-changing-after-failover). |
| Replica does not catch up | Replication credentials, network access, or clone bootstrap failed. | Check [Troubleshooting](/docs/operations/troubleshooting#replication-stopped). |

## Next steps

- [App Integration](/docs/configuration/app-integration) - Connect applications to the primary and replica Services.
- [GitOps Guidance](/docs/operations/gitops) - Configure Argo CD or Flux ownership, sync order, and health checks.
- [Failover](/docs/operations/failover) - Understand detection, promotion, DNS steering, and cooldown behavior.
- [Monitoring](/docs/observability/monitoring) - Set up Prometheus metrics and alerting.
- [Production Install](/docs/get-started/install-production) - Move from a first group to a production-ready install.
