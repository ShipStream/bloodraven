...
Bloodraven supports Dragonfly `v1.38.0+` for managed deployments. The playground pins `docker.dragonflydb.io/dragonflydb/dragonfly:v1.38.0`. The operator does not parse the image tag: on first successful connect it probes the running command table for `REPLTAKEOVER` and reports the result on `status.dragonfly.replTakeoverSupported`. An older image that lacks the command still forms a cluster; emergency promotion then falls back to `REPLICAOF NO ONE` and increments `bloodraven_dragonfly_promotions_total{result="sessions_lost"}`.
