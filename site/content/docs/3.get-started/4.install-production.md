...
Application deployment coordination is opt-in. See the [Deployment API values](/docs/configuration/deployment-api#transport-and-chart-values) for `auxiliary.deployAPI.enabled`, the required existing escrow TLS Secret, audience, and optional NetworkPolicy. Apply the matching CRD updates before declaring `MysqlDatabase.spec.deploymentClients`; Helm upgrades alone do not upgrade CRDs.
...
# Optional Organization license JWT. Public assertion, not a secret.
