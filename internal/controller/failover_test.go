...
func (t *trackingMock) GetGtidExecuted(_ context.Context) (string, error) {
	t.record("GetGtidExecuted")
	t.mu.Lock()
	defer t.mu.Unlock()
...
func TestFailoverExecute_FullSequence(t *testing.T) {
	candidate := &trackingMock{readOnly: true, gtidExecuted: "uuid1:1-10"}
	oldPrimary := &trackingMock{}
	fc := NewFailoverController(testLogger())

	promotionGtid, err := fc.Execute(context.Background(), candidate, oldPrimary, "site1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if promotionGtid != "uuid1:1-10" {
		t.Errorf("promotionGtid: got %q, want %q", promotionGtid, "uuid1:1-10")
	}
	...
	// Verify candidate sequence: drain -> stop -> reset -> getGtid -> clear super_read_only -> set read_only=0.
...
func TestFailoverExecute_OldPrimaryNil(t *testing.T) {
	...
	_, err := fc.Execute(context.Background(), candidate, nil, "site1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFailoverExecute_DrainTimeout(t *testing.T) {
	...
	candCalls := candidate.getCalls()
	if len(candCalls) != 6 {
		t.Fatalf("expected 6 calls, got %v", candCalls)
	}
	if candCalls[5] != "SetReadOnly(OFF)" {
		t.Errorf("last call should be SetReadOnly(OFF), got %q", candCalls[5])
	}
}
