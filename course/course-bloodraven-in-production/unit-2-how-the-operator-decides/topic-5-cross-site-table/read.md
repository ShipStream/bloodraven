...
`orders` has three sites: `iad` and `pdx` as `primary-candidate`, and `reader` as `role: read-only`. The counter application is writing through the `-primary` Service every second. You can now take a poll result and name each site's state — `writable`, `read-only`, `unreachable` or `unknown` — and you know a site takes `pollInterval × failureThreshold` = 2 s × 3 = **6 s** to reach `unreachable` (objective 2). What you cannot yet do is turn three of those labels into one decision. That is one function: `EvalCrossSite` in `internal/state/matrix.go`. It takes the per-site observations and the `sitePriorities` list, and returns a single `CrossSiteAction`. Everything visible about `orders` — the condition reason, the alert text, whether a promotion happens at all — falls out of it.
...
## Two documentation defects, stated plainly

`docs/docs/failover.mdx` publishes this table under *Cross-site evaluation*, as six two-column rows. Two things are wrong with it, and both will cost you in production.

| Published docs table | The code at `matrix.go` |
| --- | --- |
| `writable` / `read-only` → **Healthy** | Same |
| `unreachable` / `read-only` → **Failover** | `Reason = "Degraded"` — there is no `Failover` reason |
| `writable` / `writable` → **Split brain** | Same, `Reason = "SplitBrain"` |
| `read-only` / `read-only` → **No primary** | Same, `Reason = "NoPrimary"` |
| `unreachable` / `unreachable` → **Total loss** | Same, `Reason = "TotalLoss"` |
| *(absent)* | one writable + ≥1 unreachable → `Reason = "Degraded"` |
| *(absent from the table)* | fence-first early return, above everything |

The first defect is the missing rows. There is no published row for "the primary is fine and a peer is down", which is exactly the state `orders` sits in for the whole recovery window after a site failure. And the fence-first return does not appear in the table at all — the surrounding prose mentions that a writable non-promotable site is fenced every poll, but nothing tells you it *preempts* split brain, failover and everything below it.

The second defect is sharper. The docs name one outcome **Failover**. The code emits `Reason = "Degraded"`. That `Reason` is written verbatim onto the `Degraded` condition in `status.conditions`, with `snap.Alert` as the message. So a Prometheus or `kubectl` alerting rule matching on a condition reason of `Failover` will never fire. Not rarely — **never**. The five reason strings that actually reach `status.conditions` are exactly: `Healthy`, `Degraded`, `SplitBrain`, `NoPrimary`, `TotalLoss`. Write your rules against those.

## Worked: two states of `orders`
...
{
  "type": "match",
  "title": "Site states of orders → the Reason string that reaches status.conditions",
  "pairs": [
    {
