...
and licence status. Each of those is correct on the date above and has a decent chance of being wrong
...
| **A7** | **Recloning an encrypted site livelocks.** `UnsealReason=Clone` is not sticky, so the recipient re-seals before `CLONE INSTANCE` runs. Unit 6 topic 3. | **Fixed.** Clone unseal is sticky until bootstrap finishes; the gate is always wired. Scenario `51-encrypted-reclone` hard-asserts the unseal. | `gh issue view 144` |

### B. Where the published docs drifted from the code

The durable lesson is in the units: **the CRD is the contract and the code is the behaviour; a
documentation page is neither.** `kubectl explain` renders what the API server validates against, and
`grep` over `config/crd/bases/` tells you what shipped. These rows are the dated instances that lesson
...
| # | Pin | Value on 2026-08-13 | Re-check with |
|---|---|---|---|
...
and read the project's own licensing page for current terms and pricing. If your organisation is
planning a production dependency, that is the conversation to have before the migration plan, not
after.
...
### Deployment coordination (`/deploy/v1`)

| # | Claim | Value / verbatim quote | Source | Angle |
|---|---|---|---|---|
| 257 | The API is opt-in and served only on the shared escrow TLS listener; the chart value requires `escrowTLS` | `deployAPI: # -- Enable /deploy/v1 on the existing escrow TLS listener only. Requires auxiliary.escrowTLS.enabled and its existingSecret. enabled: false`; `escrowMux.Handle("/deploy/v1/", deployHandler)` | `charts/bloodraven/values.yaml:144-147`; `cmd/bloodraven/main.go:287, 308-310` | A6 |
| 258 | Non-leaders answer `503 not_leader` | `deployJSON(w, 503, map[string]string{"error": "not_leader"})` | `cmd/bloodraven/deploy_api.go:142, 249` | A6 |
| 259 | Token audience defaults to `bloodraven-deploy`, overridable by `BLOODRAVEN_DEPLOY_API_AUDIENCE` | `audience = "bloodraven-deploy"` | `cmd/bloodraven/deploy_api.go:45-47`; `cmd/bloodraven/main.go:308` | A6 |
| 260 | Authorization is `MysqlDatabase.spec.deploymentClients[]`, one client per namespace; `instance` is the CR name | `// DeploymentClients authorizes ServiceAccounts to use the deployment companion API for this database. Each namespace has one client.`; `Declare callers in MysqlDatabase.spec.deploymentClients; instance is the CR name.` | `api/v1alpha1/mysqldatabase_types.go:188-193`; `charts/bloodraven/templates/NOTES.txt:49` | A6 |
| 261 | `ttlSeconds` clamp `[5, 120]` on grant and renew | `time.Duration(max(5, min(120, ttl))) * time.Second` | `internal/controller/deployment_leases.go:448, 534` | A6 |
| 262 | One `migration` lease per group; a conflicting grant is `409 held` with the holder | `&DeploymentLeaseError{Status: 409, Code: "held", Holder: &old.DeploymentLeaseView}` | `internal/controller/deployment_leases.go:416-420` | A6 |
| 263 | `failover-hold` is granted only to the current migration holder, else `403` | `if migration == nil \|\| migration.State != "active" \|\| !sameDeploymentHolder(...) { ... Status: 403, Code: "forbidden" }` | `internal/controller/deployment_leases.go:399-408` | A6 |
| 264 | Renewal of an expired or released lease is `404 not_found`; a wrong token is `403 token_mismatch` | `Status: 404, Code: "not_found"`; `Status: 403, Code: "token_mismatch"` | `internal/controller/deployment_leases.go:511-526` | A6 |
| 265 | A generation change revokes with `topology_changed`; renewal then answers `409 revoked` carrying the new generation | `&DeploymentLeaseError{Status: 409, Code: "revoked", Reason: "topology_changed", TopologyGeneration: fresh.Status.TopologyGeneration}` | `internal/controller/deployment_leases.go:194, 574` | A6 |
| 266 | `423 unstable` reasons come from `DeploymentUnstableReason`; lag alone (`ReplicationLagging`) is excluded | `if name == "Degraded" && c.Reason == "ReplicationLagging" { continue }` | `internal/controller/deployment_leases.go:267-337, 395-397` | A6 |
| 267 | Every `423 unstable` carries `retryAfterSeconds: 1`; the course treats the field as advisory | `RetryAfterSeconds: 1` (every `423` site) | `internal/controller/deployment_leases.go:353-397, 578, 630-677` | A6 |
| 268 | Revocation tombstones outlive the slot until the group is deleted; a revoked operation may never re-acquire | `// Revocation tombstones intentionally live until the owning group is deleted`; `// A revoked operation may never silently acquire a new fencing epoch.` | `internal/controller/deployment_leases.go:167-175, 425-441` | A6 |
| 269 | Emergency topology changes never wait on the lease store or the admission mutex | `// pending is separate from admission: emergency topology changes never wait`; `// Lease API request must not delay the next emergency failover decision.` | `internal/controller/deployment_leases.go:80-81, 827-828` | A1/A6 |
| 270 | A live hold stamps planned failover `Deferred` / `DeploymentHold` with `retryAfter` = hold expiry, raising `PlannedFailoverDeferred` and the deferred counter | `retry := metav1.NewTime(hold.ExpiresAt)`; `Phase: v1alpha1.PlannedFailoverPhaseDeferred, Reason: reason`; `PlannedFailoversDeferredTotal.WithLabelValues(fg.Name, "DeploymentHold")` | `internal/controller/planned_failover_deployment.go:16-62` | A6 |
| 271 | `status.topologyGeneration` increments on authoritative active-site changes and is persisted | `// TopologyGeneration increments whenever the authoritative active site changes` → `TopologyGeneration int64`; bump sites in the planned, restore-in-place and topology paths | `api/v1alpha1/types.go:554-558`; `internal/controller/planned_failover_reconciler.go:911`; `internal/controller/restore_inplace.go:733`; `internal/controller/runner.go:901`; `internal/controller/topology_deployment.go:76` | A6 |
| 272 | Manual revocation annotation; renewals then report `operator_revoked` | `RevokeDeploymentLeasesAnnotation = "bloodraven.shipstream.io/revoke-deployment-leases"` | `internal/controller/deployment_leases.go:32, 725` | A6 |
| 273 | Client contract: renew at `ttl/3`; `404`/`409` or a changed generation means lost authority; before DDL abort, at/after DDL stop writes and verify by hand; a TTL is not an exactly-once DDL guarantee | verbatim in the reference page | `site/content/docs/4.configuration/11.deployment-api.md:219`; reference client `internal/playground/scenarios/deploy_client.py` | A6 |
| 274 | Chaos coverage runs the real API over TLS with projected tokens | scenarios 53-56 | `internal/playground/scenarios/s53_deployment_coordination.go`; `playground/chaos-scenarios.md` | A6 |
